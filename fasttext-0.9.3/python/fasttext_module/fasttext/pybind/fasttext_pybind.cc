...
PYBIND11_MODULE(fasttext_pybind, m) {
  ...
  py::class_<fasttext::FastText>(m, "fasttext")
    ...
    // Убедись, что <cstring> подключен для memcpy
    ...
    return py::array_t<fasttext::real>(
        ...
        copied_data_ptr,
        free_when_done
    ); // Конец конструктора py::array_t
})
      .def(
          "getOutputMatrix",
          [](fasttext::FastText& m) { // Начало лямбды
    std::shared_ptr<const fasttext::DenseMatrix> mm = m.getOutputMatrix(); // <--- Используем getOutputMatrix

    // Проверяем, что матрица не пустая
    if (!mm || mm->size(0) == 0 || mm->size(1) == 0) {
        return py::array_t<fasttext::real>(); // Возвращаем пустой numpy массив
    }
    ...
    // Выделяем новую память (Python будет ей владеть)
    fasttext::real* copied_data_ptr = new fasttext::real[num_elements];

    // Копируем данные (убедись, что #include <cstring> добавлен в начале файла)
    std::memcpy(copied_data_ptr, original_data_ptr, data_size_bytes);

    // Создаем объект-капсулу pybind11, чтобы Python удалил память
    py::capsule free_when_done(copied_data_ptr, [](void *f) {
        delete[] reinterpret_cast<fasttext::real *>(f);
    });

    // Создаем py::array_t из СКОПИРОВАННЫХ данных
    return py::array_t<fasttext::real>(
        {rows, cols},           // Форма массива
        copied_data_ptr,        // Указатель на КОПИЮ данных
        free_when_done          // Капсула для управления памятью
    ); // Конец конструктора py::array_t
})
      .def(
          "setMatrices",
